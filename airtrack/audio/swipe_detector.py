...
# ── constants ──────────────────────────────────────────────────────────────
_HPF_CUTOFF_HZ: int = 150       # remove rumble / HVAC below this frequency
_HPF_ORDER: int = 2
_PRE_ROLL_CHUNKS: int = 2       # pre-roll context before onset
_OFFSET_CONFIRM_CHUNKS: int = 4 # consecutive low-energy chunks to confirm offset
_MIN_DURATION_SEC: float = 0.06 # discard sub-60 ms events (clicks / taps)
_MAX_DURATION_SEC: float = 2.5  # force-finalize if swipe lasts > 2.5 s
...
class SwipeEvent:
    ...
    Attributes:
        audio: Float32 mono array (high-pass filtered).
        duration_sec: Duration in seconds.
        peak_rms: Peak RMS across 256-sample windows inside the swipe.
        timestamp: Monotonic time at detection (seconds).
        wav_path: Path where the raw audio was saved (set after :func:`save_wav`).
    """
...
def _make_hpf(sample_rate: int) -> np.ndarray:
    """Return a second-order-sections Butterworth high-pass filter."""
    nyq = sample_rate / 2.0
    return butter(_HPF_ORDER, _HPF_CUTOFF_HZ / nyq, btype="high", output="sos")


def save_wav(path: str, audio: np.ndarray, sample_rate: int) -> None:
    """Write a float32 audio array to a 16-bit WAV file.
...
def load_wav(path: str) -> tuple[np.ndarray, int]:
    """Load a 16-bit WAV file as a float32 array.

    Returns:
        Tuple of (audio float32 in [-1,1], sample_rate).
    """
...
class SwipeDetector:
    """Processes audio chunks in real-time and emits :class:`SwipeEvent` objects.

    Args:
        sample_rate: Audio sample rate (must match the recorder).
        noise_floor_rms: Ambient RMS measured during the calibration step.
        onset_multiplier: onset threshold = noise_floor × this value.
        offset_multiplier: offset threshold = noise_floor × this value.
        save_dir: Directory where detected-swipe WAV files are written.
    """
    ...
    def process_chunk(self, chunk: np.ndarray) -> Optional[SwipeEvent]:
        ...
        Args:
            chunk: Float32 mono array of length CHUNK_SIZE.

        Returns:
            A :class:`SwipeEvent` when a swipe is finalized, else None.
        """
        ...
        if self._state == _State.IDLE:
            ...
            if rms > self._onset_thr:
                ...
                self._buffer = list(self._pre_roll)  # include pre-roll
            ...
            if rms < self._offset_thr:
                ...
                # Energy came back — still swiping
                self._state = _State.COLLECTING
                self._offset_count = 0
    ...
    def reset(self) -> None:
        """Reset the state machine to IDLE without touching the filter or index."""
        self._state = _State.IDLE
        self._buffer = []
        self._pre_roll.clear()
        self._offset_count = 0
    ...
    def _compute_peak_rms(audio: np.ndarray, window: int = 256) -> float:
        """Return the peak RMS over sliding windows."""
        n = len(audio)
        if n < window:
